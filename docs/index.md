# Programación de Inteligencia Artificial

Bienvenido al portal público del módulo **Programación de Inteligencia Artificial (PIA)** para el curso 2026/2027.

## Propósito

Este espacio reúne los materiales de trabajo para el alumnado: teoría, ejemplos, prácticas, recursos y guías de proyecto. La publicación está pensada para consultar las unidades de forma progresiva y mantener fuera del sitio público la planificación interna, los materiales sensibles, los archivos históricos y los recursos exclusivos del profesorado.

## Unidades

1. [UD1 — Fundamentos de Python](unidades/ud01.md)
2. [UD2 — Tratamiento de Datos](unidades/ud02.md)
3. [UD3 — Machine Learning](unidades/ud03.md)
4. [UD4 — Deep Learning](unidades/ud04.md)
5. [UD5 — Cloud y MLOps](unidades/ud05.md)
6. [UD6 — LLM y Agentes](unidades/ud06.md)
7. [UD7 — Convergencia de Herramientas](unidades/ud07.md)
8. [UD8 — Visión y XAI](unidades/ud08.md)
9. [UD9 — GPU Avanzado](unidades/ud09.md)
10. [UD10 — Series Temporales](unidades/ud10.md)
11. [UD11 — Anexos](unidades/ud11.md)
12. [UD12 — Proyecto Integrado](unidades/ud12.md)

## Proyecto final

El módulo culmina con un proyecto integrado en equipo. El alumnado deberá plantear una solución basada en datos e IA, validar el alcance, integrar fuentes de datos, construir una salida útil y defender las decisiones técnicas tomadas durante el desarrollo.

Consulta la [guía del Proyecto Integrado](unidades/ud12.md) para acceder al material principal y a los checkpoints del proyecto.

## Normas comunes de entrega

Todas las tareas, prácticas y proyectos con entrega están sujetos a las [normas comunes de entrega y uso de inteligencia artificial](normas-entregas-y-uso-de-ia.md). En ellas se explican la penalización gradual por retraso, las evidencias que deben aportarse cuando se utilice IA y cómo demostrar que el resultado se ha verificado.

## Portal común

Está prevista la creación de un portal común para coordinar la publicación de materiales de **SBD**, **PIA** y **DWES** desde un punto de acceso compartido.
